...
class TestAnalysisEndpoint:
    ...
    def test_analyze_success(self, client, sample_image_bytes):
    ...
    def test_get_task_status_not_found(self, client):
        ...
        response = client.get(
            "/api/v1/tasks/nonexistent-task-id", headers={"X-API-Key": "changeme"}
        )
    ...
    def test_get_result_not_found(self, client):
        ...
        response = client.get(
            "/api/v1/result/nonexistent-task-id", headers={"X-API-Key": "changeme"}
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
